	...
	public class CalculateService : ICalculateService
	{
		...
		public IEnumerable<DrugUnit> SearchDrugUnits(int depotId, List<int> numbers)
		{
			...
			for (int i = 0; i < numbers.Count(); i++)
			{
				...
				{
					if (j < (drugUnitRepository.GetAll().Where(x => x.DepotId == depotId && x.Id == (i + 1))).ToList<DrugUnit>().Count)
						units.Add((drugUnitRepository.GetAll().Where(x => x.DepotId == depotId && x.Id == (i + 1))).ToList<DrugUnit>().OrderBy(x => x.PickNumber).ElementAt(j));
				}
			}
			return units;
		}
